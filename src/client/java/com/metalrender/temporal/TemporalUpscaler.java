...
/**
 * Maintains a soft target resolution scale when temporal anti-aliasing is enabled. The upscaler nudges the current
 * render scale towards the configured temporal target every frame, allowing the dynamic quality controller to keep the
 * frame rate stable without fighting the temporal resolve.
 */

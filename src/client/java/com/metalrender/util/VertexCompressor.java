...
public final class VertexCompressor {
    ...
    public static CompressedMesh compress(BlockPos origin, ByteBuffer source, int vertexCount) {
        ...
        ByteBuffer target = ByteBuffer.allocateDirect(vertexCount * OUTPUT_STRIDE).order(ByteOrder.LITTLE_ENDIAN);
        ...
        for (int vertexIndex = 0; vertexIndex < vertexCount; vertexIndex++) {
            ...
            int normalPacked = packNormal(input.get(base + 28), input.get(base + 29), input.get(base + 30));
            int color = input.getInt(base + 12);
            int uvPacked = packHalf2(input.getFloat(base + 16), input.getFloat(base + 20));
            int lightPacked = packLight(input.getShort(base + 24), input.getShort(base + 26));

            target.putShort(qx);
            target.putShort(qy);
            target.putShort(qz);
            target.putShort((short) 0); // reserved for future attributes (e.g., tangent)
    ...
    public static final class Bounds {
        ...
        private Bounds(float minX, float minY, float minZ, float maxX, float maxY, float maxZ, float radius) {
            this.minX = minX;
            this.minY = minY;
            this.minZ = minZ;
            this.maxX = maxX;
        ...
        private static Bounds from(float minX, float minY, float minZ, float maxX, float maxY, float maxZ) {

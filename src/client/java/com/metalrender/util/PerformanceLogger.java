...
public final class PerformanceLogger {
    ...
    private void logPerformanceStats() {
        double cullingEfficiency = this.totalChunksProcessed > 0L
            ? (double) (this.totalFrustumCulled + this.totalOcclusionCulled) / (double) this.totalChunksProcessed
                * 100.0D
            : 0.0D;
        MetalLogger.info("[PERF] FPS: %.1f | FrameTime: %.2fms | Chunks: P:%d D:%d | Culled: F:%d O:%d (%.1f%%)",
            this.currentFPS, this.avgFrameTime, this.totalChunksProcessed, this.totalChunksDrawn,
            this.totalFrustumCulled, this.totalOcclusionCulled, cullingEfficiency);

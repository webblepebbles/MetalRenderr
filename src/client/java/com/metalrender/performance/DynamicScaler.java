...
public final class DynamicScaler {
    ...
    public void onFrameEnd(double frameMs) {
        ...
        if (ratio > 2.5) {
            criticalFrameCount++;
            if (criticalFrameCount > EMERGENCY_THRESHOLD_FRAMES && !emergencyTriggered) {
                enableEmergencyMode();
            }
        } else if (ratio < 1.5) {
        ...
        int maxSimulationDistance = Math.max(minSimulationDistance, MetalRenderConfig.dqMaxSimulationDistance());
        ...
        } else if (ratio < 0.8 && currScale >= maxScale - 0.05f && fastFrameStreak > 3) {
        ...
        if (simulationChanged && currSimDist != lastSimulation) {
            ...
            if (client.getServer() != null && client.getServer().getPlayerManager() != null) {
                client.getServer().getPlayerManager().setSimulationDistance(currSimDist);
            }
            lastSimulation = currSimDist;
            MetalLogger.debug(
                "[DQ] Adjusted simulation distance to %d (ratio=%.2f, scale=%.2f)", currSimDist, ratio, currScale);
        }
    ...
    private void ensureUncapped(MinecraftClient client) {
        ...
        try {
            ...
            if (!uncapWarned) {
                MetalLogger.warn("[DQ] Unable to disable VSync directly: {}", t.toString());
            }
            uncapWarned = true;
        }
        ...
        if (client.options != null) {
            if (setSimpleOption(client.options, new String[] {"getEnableVsync", "getEnableVSync"}, Boolean.FALSE)) {
                updatedLimits = true;
            }
            if (setSimpleOption(client.options, new String[] {"getFramerateLimit", "getFrameRateLimit", "getMaxFps"},
                    Integer.valueOf(260))) {
                updatedLimits = true;
            }
        }
        ...
        if (updatedLimits && !vsyncUnlocked) {
            vsyncUnlocked = true;
            MetalLogger.info("[DQ] Forced VSync off and raised FPS cap to 260 for high-performance scaling");
        }
    }

    private void logFrameStats(float scale, int viewDistance, int simulationDistance, double ratio) {
        long now = System.currentTimeMillis();
        ...
        MetalLogger.info(
            "[DQ][Stats] FPS=%.1f | Scale=%.2f | Ratio=%.2f | View=%d | Sim=%d | Emergency=%s | Critical=%d", fps,
            scale, ratio, viewDistance, simulationDistance, emergencyTriggered ? "on" : "off", criticalFrameCount);
    }

    @SuppressWarnings("unchecked")
    private boolean setSimpleOption(Object options, String[] methodNames, Object value) {
    ...
    private void enableEmergencyMode() {
        ...
        MetalLogger.info("[DQ] EMERGENCY MODE ENABLED - disabling expensive features");
    }

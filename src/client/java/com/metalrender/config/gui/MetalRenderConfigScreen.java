...
public final class MetalRenderConfigScreen extends Screen {
    ...
    protected void init() {
        ...
        this.metalRenderToggle = addToggle(left, y, toggleWidth, "MetalRender", data.metalRenderEnabled, value -> {
            MetalLogger.info("[Config UI] MetalRender: %s -> %s", data.metalRenderEnabled, value);
            MetalRenderConfigManager.update(cfg -> cfg.metalRenderEnabled = value);
            MetalRenderClient.refreshEnabledState();
        });
        y += 24;
        this.dynamicQualityToggle = addToggle(left, y, toggleWidth, "Dynamic Quality", data.dynamicQuality, value -> {
            MetalLogger.info("[Config UI] Dynamic Quality: %s -> %s", data.dynamicQuality, value);
            MetalRenderConfigManager.update(cfg -> cfg.dynamicQuality = value);
        });
        this.meshShaderToggle = addToggle(right, y, toggleWidth, "Mesh Shaders", data.meshShadersEnabled, value -> {
            MetalLogger.info("[Config UI] Mesh Shaders: %s -> %s", data.meshShadersEnabled, value);
            MetalRenderConfigManager.update(cfg -> cfg.meshShadersEnabled = value);
            scheduleChunkReload();
        });
        y += 24;
        this.lodToggle = addToggle(left, y, toggleWidth, "LOD System", data.distanceLodEnabled, value -> {
            MetalLogger.info("[Config UI] LOD System: %s -> %s", data.distanceLodEnabled, value);
            MetalRenderConfigManager.update(cfg -> cfg.distanceLodEnabled = value);
            scheduleChunkReload();
        });
        this.occlusionToggle = addToggle(right, y, toggleWidth, "Occlusion Culling", data.occlusionCulling, value -> {
            MetalLogger.info("[Config UI] Occlusion Culling: %s -> %s", data.occlusionCulling, value);
            MetalRenderConfigManager.update(cfg -> cfg.occlusionCulling = value);
            scheduleChunkReload();
        });
        y += 24;
        this.temporalToggle = addToggle(left, y, toggleWidth, "Temporal AA", data.temporalAAEnabled, value -> {
            MetalLogger.info("[Config UI] Temporal AA: %s -> %s", data.temporalAAEnabled, value);
            MetalRenderConfigManager.update(cfg -> cfg.temporalAAEnabled = value);
        });
        this.frustumToggle =
            addToggle(right, y, toggleWidth, "Aggressive Frustum", data.aggressiveFrustumCulling, value -> {
                MetalLogger.info("[Config UI] Aggressive Frustum: %s -> %s", data.aggressiveFrustumCulling, value);
                MetalRenderConfigManager.update(cfg -> cfg.aggressiveFrustumCulling = value);
            });
        y += 24;
        this.mirrorUploadsToggle = addToggle(left, y, toggleWidth, "Mirror Uploads", data.mirrorUploads, value -> {
            MetalLogger.info("[Config UI] Mirror Uploads: %s -> %s", data.mirrorUploads, value);
        ...
        this.dqTargetSlider = addSlider(sliderX, sliderY, sliderWidth, "Dynamic Target Frame (ms)", 6.0D, 25.0D, 0.5D,
            data.dqTargetFrameMs, value -> {
                MetalLogger.info("[Config UI] Dynamic Target Frame: %.2fms -> %.2fms", data.dqTargetFrameMs, value);
                MetalRenderConfigManager.update(cfg -> cfg.dqTargetFrameMs = value);
            });
        sliderY += 24;

        this.dqMinSlider = addSlider(
            sliderX, sliderY, sliderWidth, "Min Resolution Scale", 0.5D, 1.0D, 0.01D, data.dqMinScale, value -> {
                MetalLogger.info("[Config UI] Min Resolution Scale: %.2f -> %.2f", data.dqMinScale, value);
        ...
        this.dqMaxSlider = addSlider(
            sliderX, sliderY, sliderWidth, "Max Resolution Scale", 0.7D, 1.25D, 0.01D, data.dqMaxScale, value -> {
                MetalLogger.info("[Config UI] Max Resolution Scale: %.2f -> %.2f", data.dqMaxScale, value);
                MetalRenderConfigManager.update(cfg -> {
        ...
        this.dqStepSlider =
            addSlider(sliderX, sliderY, sliderWidth, "Scale Step", 0.01D, 0.25D, 0.01D, data.dqScaleStep, value -> {
                MetalLogger.info("[Config UI] Scale Step: %.2f -> %.2f", data.dqScaleStep, value);
                MetalRenderConfigManager.update(cfg -> cfg.dqScaleStep = (float) value);
            }, v -> String.format("%.2f", v));
        sliderY += 24;

        this.temporalScaleSlider = addSlider(sliderX, sliderY, sliderWidth, "Temporal Upscale Target", 0.75D, 1.0D,
            0.01D, data.temporalUpscaleTarget, value -> {
                MetalLogger.info(
                    "[Config UI] Temporal Upscale Target: %.2f -> %.2f", data.temporalUpscaleTarget, value);
                MetalRenderConfigManager.update(cfg -> cfg.temporalUpscaleTarget = (float) value);
            }, v -> String.format("%.2f", v));
        sliderY += 24;

        this.temporalBlendSlider = addSlider(sliderX, sliderY, sliderWidth, "Temporal Blend Factor", 0.05D, 0.30D,
            0.01D, data.temporalBlendFactor, value -> {
                MetalLogger.info("[Config UI] Temporal Blend Factor: %.2f -> %.2f", data.temporalBlendFactor, value);
                MetalRenderConfigManager.update(cfg -> cfg.temporalBlendFactor = (float) value);
            }, v -> String.format("%.2f", v));
        sliderY += 24;

        this.lodDistantScaleSlider = addSlider(
            sliderX, sliderY, sliderWidth, "Distant LOD Scale", 0.10D, 1.0D, 0.01D, data.lodDistantScale, value -> {
                MetalLogger.info("[Config UI] Distant LOD Scale: %.2f -> %.2f", data.lodDistantScale, value);
        ...
        ButtonWidget resetButton = ButtonWidget
                                       .builder(Text.literal("Reset to Defaults"),
                                           btn -> {
                                               MetalLogger.info("[Config UI] Resetting all settings to defaults");
        ...
        ButtonWidget doneButton = ButtonWidget.builder(Text.translatable("gui.done"), btn -> close())
    ...
    private ToggleControl addToggle(
        int x, int y, int width, String label, boolean initial, Consumer<Boolean> listener) {
        ToggleControl control = new ToggleControl(label, initial, listener);
        ButtonWidget widget =
            ButtonWidget.builder(Text.empty(), btn -> control.toggle()).position(x, y).size(width, 20).build();
        control.attach(widget);
        this.addDrawableChild(widget);
        return control;
    }

    private RangeSlider addSlider(int x, int y, int width, String label, double min, double max, double step,
        double initial, DoubleConsumer listener) {
        return addSlider(x, y, width, label, min, max, step, initial, listener, null);
    }

    private RangeSlider addSlider(int x, int y, int width, String label, double min, double max, double step,
        double initial, DoubleConsumer listener, Function<Double, String> formatter) {
        RangeSlider slider = new RangeSlider(label, x, y, width, min, max, step, initial, listener, formatter);
    ...
    private final class ToggleControl {
        ...
        private void updateLabel() {
            if (this.button != null) {
                this.button.setMessage(Text.literal(this.label + ": " + (this.value ? "ON" : "OFF")));
            }
        }
    }
    ...
    private final class RangeSlider extends SliderWidget {
        ...
        private RangeSlider(String label, int x, int y, int width, double min, double max, double step,
            double currentValue, DoubleConsumer listener, Function<Double, String> formatter) {
        ...
        private void setRealValue(double value) {
            this.value = normalize(this.min, this.max, MathHelper.clamp(value, this.min, this.max));
            updateMessage();
        }
    }

...
public final class MetalRenderConfigManager {
    ...
    private static final Path CONFIG_PATH = FabricLoader.getInstance().getConfigDir().resolve("metalrender.json");
    ...
    public static synchronized void load() {
        ...
        if (Files.exists(CONFIG_PATH)) {
            try (Reader reader = Files.newBufferedReader(CONFIG_PATH)) {
                ...
                MetalLogger.warn("Failed to load config, using defaults (%s)", e.getMessage());
            }
        }
    ...
    public static synchronized void update(Consumer<MetalRenderConfigData> mutator) {

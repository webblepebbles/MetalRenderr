...
public final class MetalRenderer implements RenderBackend {
   ...
   public long createVertexBuffer(ByteBuffer data, int size, int stride) {
      return data != null && size > 0 ? MetalBackend.createVertexBuffer(this.handle, data, size) : 0L;
   }

   public long createIndexBuffer(ByteBuffer data, int size) {
      return data != null && size > 0 ? MetalBackend.createIndexBuffer(this.handle, data, size) : 0L;
   }
   ...
   public void drawIndexed(long vbo, long ibo, int indexCount, int firstIndex, int baseVertex) {
      int pass = 0;
      if (this.pipelinesReady && MetalBackend.isPipelineReady(this.handle, pass)) {
         MetalBackend.drawIndexed(this.handle, vbo, ibo, indexCount, firstIndex, baseVertex, pass);
      }

   }

   public void endFrame() {
      MetalBackend.render(this.handle, 0.0F);
   }

   public boolean isPassReady(int pass) {
      return this.pipelinesReady && MetalBackend.isPipelineReady(this.handle, pass);
   }

   public boolean isOpaqueReady() {
      return this.isPassReady(0);
   }
}

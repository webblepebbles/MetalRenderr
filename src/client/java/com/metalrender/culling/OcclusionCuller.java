...
public final class OcclusionCuller {
    ...
    public OcclusionCuller() {
        this.aabbBytes =
            ByteBuffer.allocateDirect(MAX_QUERIES * COMPONENTS_PER_AABB * Float.BYTES).order(ByteOrder.nativeOrder());
        this.aabbFloats = this.aabbBytes.asFloatBuffer();
        this.resultBytes = ByteBuffer.allocateDirect(MAX_QUERIES).order(ByteOrder.nativeOrder());
    }

    public void beginFrame(long ctxHandle, Camera camera, float[] viewProj, int width, int height) {
        ...
        if (this.hiZBuffer.ensure(ctxHandle, Math.max(1, width), Math.max(1, height))) {
            NativeBridge.nOcclusionBegin(ctxHandle, this.hiZBuffer.getHandle(), viewProj);
        }
    }

    public boolean isChunkOccluded(BlockPos chunkPos, int minBlockY, int maxBlockY) {
    ...
    public void resolve() {
        if (!this.supported || this.pending.isEmpty() || this.contextHandle == 0L || this.hiZBuffer.getHandle() == 0L) {
            return;
        }
        ...
        NativeBridge.nOcclusionEvaluate(
            this.contextHandle, this.hiZBuffer.getHandle(), this.aabbFloats, queryCount, this.resultBytes);
    ...
    private void sweepCache() {
        ...
        this.queryCache.values().removeIf(query -> query.lastSubmittedFrame < threshold);
    }
